...
function onCreate(){
	var bg = new BGSprite("stages/philly/sky", -100, 0, 0.1, 0.1);
	insert(members.indexOf(gfGroup), bg);
	
	
	var bg = new BGSprite("stages/philly/city", -255, 45, 0.3, 0.3);
	...
	var bg = new BGSprite("stages/philly/behindTrain", -299, 144);
	insert(members.indexOf(gfGroup), bg);
	spritePhillyGlow.push(bg);
	...
	var bg = new BGSprite("stages/philly/street", -299, 144);
	insert(members.indexOf(gfGroup), bg);
...
function doWinlight() {
	window.alpha = 1;
	var windowColor = [0xFF31A2FD, 0xFF31FD8C, 0xFFFB33F5, 0xFFFBA633, 0xFFFD4531][FlxG.random.int(0,4)];
	gradient.height = 400;
